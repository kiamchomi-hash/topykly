...
description: Deep OWASP-pattern security code review for vulnerabilities in this project's JS/Node code (injection, XSS, authentication, authorization, CSRF, SSRF, cryptography, business logic). Use for a more thorough pass than the built-in security-review skill when reviewing auth, sessions, admin/moderation, or backend-store.js/preview-server.js changes — e.g. "do a deep security review of the auth changes", "audit this for OWASP issues". Provides systematic, confidence-based reporting so it only flags exploitable, attacker-controlled issues.
allowed-tools: Read, Grep, Glob, Bash, Task
license: LICENSE
---

<!--
Vendored from https://github.com/getsentry/skills (skills/security-review), trimmed to the
JavaScript/Node guide and the vulnerability references relevant to this project (no Python/Go/
Rust/Java or Docker/Kubernetes/Terraform/cloud guides, since this repo has none of that).
Reference material based on OWASP Cheat Sheet Series (CC BY-SA 4.0): https://cheatsheetseries.owasp.org/
-->

# Security Review Skill (OWASP, deep pass)
...
This project is plain ES modules (`.js`) on Node, no framework — always load `languages/javascript.md`.
...
- `javascript.md` - Node, Express, React, Vue, Next.js (the only language guide kept; this project has no Python/Go/Rust/Java code)
