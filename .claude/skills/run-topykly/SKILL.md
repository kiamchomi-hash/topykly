...
- **Quick "logged in as a demo user" path**: click the auth button in the topbar and use the plain login action with no provider configured — the backend falls back to a fixed seeded registered user (`u1`, "Coco Mora") via `store.login()`. No env vars needed. Good for chat/ranking/notifications/friends flows that don't depend on a specific identity.
  - **Gotcha**: the repo's `.env` has real `TOPYKLY_OIDC_*` credentials, so this fallback does NOT work out of the box — the auth button redirects to Google instead. `loadEnvFile` skips keys already present in `process.env`, so for an isolated verification instance launch through a small wrapper that presets those keys to `""` (plus `TOPYKLY_SEED_DEMO_DATA=1` so seed user `u1` exists, and `TOPYKLY_DB_PATH` to a scratch file) before importing `services/preview-server.js`.
  - For browser automation without a local playwright install: `npx --no-install playwright --version` reveals a cached copy under `%LOCALAPPDATA%\npm-cache\_npx\<hash>\node_modules\playwright` that can be imported by absolute `file:///` URL — pick the `<hash>` whose `playwright-core/browsers.json` chromium revision matches an installed `%LOCALAPPDATA%\ms-playwright\chromium-*` folder.
