...
2. **Estos 7 días se midieron SIN filtro de bots**, así que el denominador está inflado y toda tasa de conversión de esta tabla está subestimada. El filtro se agregó el mismo 2026-07-22 (§7, ítem 2), **después** de esta captura: los datos de acá en adelante no son directamente comparables con los de esta tabla, y lo esperable es que el conteo de visitantes **baje** sin que eso signifique perder tráfico real.
...
2. **[HECHO el 2026-07-22]** Filtro de tráfico automático en `product_events`. El ingest (`POST /api/events`) descarta rastreadores declarados, automatización evidente (headless, `curl`, clientes HTTP) y peticiones sin user agent; responde 202 igual para no darle señal al rastreador, pero no registra el evento **ni persiste sesión de invitado**, así que tampoco inflan la presencia. Lógica en `shouldCountProductEvent` (`services/preview-server.js`), cubierta por tests unitarios y de humo.

   **Límite conocido, tenerlo presente al leer:** el filtro se basa en el user agent, así que **no atrapa automatización que se disfraza de Chrome**. El pico del 18-19 de julio (§3) podría no haber sido filtrado por esto. Si vuelve a aparecer una cohorte con la misma firma —1 `page_view` por sujeto, sin `topic_open`, sin regresos—, tratarla como sospechosa igual.
